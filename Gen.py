...
def TitleWorkerr():
    ...
    if sys.platform != "linux" or sys.platform != "darwin":
        ctypes.windll.kernel32.SetConsoleTitleW(f'[Space Generator v2 | {config["discord"]["invite_code"]}] | Generated : {genned} | Errors : {errors} | Solved : {solved} | Speed : {round(genned / ((time.time() - genStartTime) / 60))}/m')
...
class Utils(object):
    ...
    @staticmethod
    def GetSuper():
      with open('input/xsup.txt', "r") as f:
        return random.choice(f.readlines()).strip()
